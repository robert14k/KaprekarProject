...
int isKaprekar(int n) {
  ...
  if(n < 1) {
    return 0; //change false to 0;
  }

  int i;
  long square = n * (long) n;
  int numDigits = (int) log10(n) + 1;
  long modulus = 1; //can't divide by 0
  long first, second;

  //for each possible "split" of the square...
  for(i=1; i<=numDigits; i++) {//change to numDigits {
    //increase the modulus by a factor of 10
    modulus *= 10; //change to modulus / spelling 
	...
	return 0;
   //move return up a bracket and erased one 
}